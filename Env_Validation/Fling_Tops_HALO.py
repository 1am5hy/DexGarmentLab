...
class FlingTops_Env(BaseEnv):
    def __init__(
        ...
        # if collect data, load judge_camera and judge_garment to validation_flag the success of final state
        ...
        self.sadp_g_stage_1 = SADP_G(task_name="Fling_Tops_stage_1", data_num=training_data_num, checkpoint_num=stage_1_checkpoint_num)

        self.sadp_g_stage_2 = SADP_G(task_name="Fling_Tops_stage_2", data_num=training_data_num, checkpoint_num=stage_2_checkpoint_num)
...
def FlingTops(pos, ori, usd_path, ground_material_usd, validation_flag, record_vedio_flag, training_data_num, stage_1_checkpoint_num, stage_2_checkpoint_num, stage_3_checkpoint_num):

    env = FlingTops_Env(pos, ori, usd_path, ground_material_usd, record_vedio_flag, training_data_num, stage_1_checkpoint_num, stage_2_checkpoint_num, stage_3_checkpoint_num)
    ...
    if record_vedio_flag:
        if not os.path.exists("Data/Fling_Tops_Validation_HALO/vedio"):
            os.makedirs("Data/Fling_Tops_Validation_HALO/vedio")
        env.env_camera.create_mp4(get_unique_filename("Data/Fling_Tops_Validation_HALO/vedio/vedio", ".mp4"))
    ...
    if validation_flag:
        if not os.path.exists("Data/Fling_Tops_Validation_HALO"):
            os.makedirs("Data/Fling_Tops_Validation_HALO")
        # write into .log file
        with open("Data/Fling_Tops_Validation_HALO/validation_log.txt", "a") as f:
            f.write(f"result:{success}  usd_path:{env.garment.usd_path}  pos_x:{pos[0]}  pos_y:{pos[1]}  ori_angle:{ori[0]}\n")
        if not os.path.exists("Data/Fling_Tops_Validation_HALO/final_state_pic"):
            os.makedirs("Data/Fling_Tops_Validation_HALO/final_state_pic")
        env.env_camera.get_rgb_graph(save_or_not=True,save_path=get_unique_filename("Data/Fling_Tops_Validation_HALO/final_state_pic/img",".png"))
...
if __name__=="__main__":
    ...
    FlingTops(pos, ori, usd_path, args.ground_material_usd, args.validation_flag, args.record_vedio_flag, args.training_data_num, args.stage_1_checkpoint_num, args.stage_2_checkpoint_num, args.stage_3_checkpoint_num)
