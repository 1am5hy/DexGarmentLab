...
DEFAULT_OVERLAP_OFFSET = 0.02
WORLD_USD_OVERLAP_OFFSET = 0.03
COLLISION_FILTER_OFFSET = 3.0

GARMENT_TARGET_CANDIDATES = (
    "Garment/geometry/mesh",
...
class ParticleSceneCfg(InteractiveSceneCfg):
    ...
    # tiled_camera = TiledCameraCfg(
    #     prim_path="/World/envs/env_.*/Camera",
    #     offset=TiledCameraCfg.OffsetCfg(pos=(-0.0, 0.5, 0.75), rot=(0, 0,  0.258819, 0.9659258), convention="opengl"),
    #     data_types=["distance_to_image_plane", "instance_id_segmentation_fast"],
    #     spawn=sim_utils.PinholeCameraCfg(
    #         focal_length=12.0, focus_distance=400.0, horizontal_aperture=20.955, clipping_range=(0.1, 20.0)
    #     ),
    ...
    sphere1 = RigidObjectCfg(
        ...
        init_state=RigidObjectCfg.InitialStateCfg(pos=(0. ,-0.2, 0.))
    )
...
def _is_valid_prim(stage, prim_path: str) -> bool:
    prim = stage.GetPrimAtPath(prim_path)
    return prim.IsValid()


def _supports_particle_attachment(stage, prim_path: str) -> bool:
    prim = stage.GetPrimAtPath(prim_path)
    if not prim.IsValid():
        return False
    apis = prim.GetAppliedSchemas()
    if any(k in api for api in apis for k in ("Particle", "Deformable", "Cloth")):
        return True
    for child in Usd.PrimRange(prim):
        child_apis = child.GetAppliedSchemas()
        if any(k in api for api in child_apis for k in ("Particle", "Deformable", "Cloth")):
            return True
    return False


def _resolve_garment_target(stage, sphere_prim_path: str, prefer_cloth: bool = False) -> Optional[str]:
    env_ns = sphere_prim_path.split("/Sphere", maxsplit=1)[0]
    ordered_candidates = list(GARMENT_TARGET_CANDIDATES)
    if prefer_cloth:
        ordered_candidates = [
            "Garment/geometry/Geom/Towel",
            "Garment/geometry/mesh",
    ...
    for rel in ordered_candidates:
        candidate = f"{env_ns}/{rel}"
        if _is_valid_prim(stage, candidate) and _supports_particle_attachment(stage, candidate):
            return candidate
    return None


def _pick_overlap_offset(garment_target: str) -> float:
    if "Geom/Towel" in garment_target:
        return WORLD_USD_OVERLAP_OFFSET
    return DEFAULT_OVERLAP_OFFSET


def _define_attachment(stage, attachment_path: str, actor0: str, actor1: str, overlap_offset: float):
    attachment = PhysxSchema.PhysxPhysicsAttachment.Define(stage, attachment_path)
    attachment.GetActor0Rel().SetTargets([actor0])
    attachment.GetActor1Rel().SetTargets([actor1])
    api = PhysxSchema.PhysxAutoAttachmentAPI(attachment.GetPrim())
    api.Apply(attachment.GetPrim())
    _ = api.CreateCollisionFilteringOffsetAttr(defaultValue=COLLISION_FILTER_OFFSET)
    _ = api.CreateDeformableVertexOverlapOffsetAttr(defaultValue=overlap_offset)


def create_attachment(attachment_prim, attachment_prim2, stage, custom=False, created_paths: Optional[Set[str]] = None):
    sphere1_paths = attachment_prim.root_physx_view.prim_paths
    sphere2_paths = attachment_prim2.root_physx_view.prim_paths
    prefer_cloth = custom == "Cloth"

    for env_idx, (sphere1_path, sphere2_path) in enumerate(zip(sphere1_paths, sphere2_paths)):
        garment_target = _resolve_garment_target(stage, sphere1_path, prefer_cloth=prefer_cloth)
        if garment_target is None:
            print(f"[ATTACH][env_{env_idx}] Skip: no valid garment target for {sphere1_path}")
            continue
        if not _is_valid_prim(stage, sphere1_path) or not _is_valid_prim(stage, sphere2_path):
            print(f"[ATTACH][env_{env_idx}] Skip: invalid sphere prim(s) {sphere1_path} | {sphere2_path}")
            continue

        overlap_offset = _pick_overlap_offset(garment_target)
        attachment_path1 = f"{garment_target}/attachment_1"
        attachment_path2 = f"{garment_target}/attachment_2"

        _define_attachment(stage, attachment_path1, garment_target, sphere1_path, overlap_offset)
        _define_attachment(stage, attachment_path2, garment_target, sphere2_path, overlap_offset)

        if created_paths is not None:
            created_paths.add(attachment_path1)
            created_paths.add(attachment_path2)
        print(f"[ATTACH][env_{env_idx}] Attached via {garment_target} (overlap={overlap_offset:.3f})")
...
def delete_attachment(scene, created_paths: Optional[Set[str]] = None):
    if created_paths:
    ...
    for i in range(scene.num_envs):
        for parent in candidate_parents:
            delete_prim(f"/World/envs/env_{i}/{parent}/attachment_1")
            delete_prim(f"/World/envs/env_{i}/{parent}/attachment_2")
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, save):
    ...
    count = 0
    pending_attach_steps = set()
    created_attachment_paths: Set[str] = set()
    attach_metrics: Dict[str, int] = {
        "checks": 0,
        "successful_pairs": 0,
        "total_pairs": 0,
    }
    ...
    while simulation_app.is_running():
        ...
        if count % 1100 == 0:
            ...
            """
            "Garment"
            delete_attachment(scene, created_attachment_paths)
            pending_attach_steps.clear()
        ...
        if count == 43:
            ...
            # garment.write_nodal_velocity_to_sim(init_vel)
            pending_attach_steps.add(count + ATTACHMENT_SETTLE_STEPS)

        if count in pending_attach_steps:
            garment_entity_cfg = SceneEntityCfg("particle_obj")
            garment_entity_cfg.resolve(scene)
            sphere = scene["sphere1"]
            sphere2 = scene["sphere2"]
            ...
            create_attachment(sphere, sphere2, stage, created_paths=created_attachment_paths)
            pending_attach_steps.remove(count)
            ...
            if count == 50 + pnp_len + 2:
                ...
                pick_position = pick_position * 0.85
        ...
        if count == 50 + 80 or count == 50 + pnp_len + 50 + 80:
            pair_success = int(attach_state[0].sum().item())
            pair_total = int(attach_state[0].numel())
            attach_metrics["checks"] += 1
            attach_metrics["successful_pairs"] += pair_success
            attach_metrics["total_pairs"] += pair_total
            current_rate = pair_success / max(pair_total, 1)
            cumulative_rate = attach_metrics["successful_pairs"] / max(attach_metrics["total_pairs"], 1)
            print(
                f"[ATTACH_METRICS] check={attach_metrics['checks']} "
                f"pair_success={pair_success}/{pair_total} "
                f"current_rate={current_rate:.3f} cumulative_rate={cumulative_rate:.3f}"
            )
...
def main():
    """Main function."""
    # Load kit helper
    sim_cfg = sim_utils.SimulationCfg(dt=0.05, device=args_cli.device)
