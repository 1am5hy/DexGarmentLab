...
class AttachmentBlock:
    def __init__(self, stage, scene, prim_path="/World/AttachmentBlock", garment_path=None):
        '''
        Args:
    ...
    def create_block(self, block_name, block_position=np.array([0.0, 0.0, 1.0]), block_visible=True):
        ...
        self.scene._add_entities_from_cfg(self.block)
    ...
    def attach(self, block_index_indices: list):
        ...
        for block_index in block_index_indices:
            for i in range(self.garment_num):
                ...
                attachment = PhysxSchema.PhysxPhysicsAttachment.Define(self.stage,
                                                                       self.attachment_path_list[block_index][i])
