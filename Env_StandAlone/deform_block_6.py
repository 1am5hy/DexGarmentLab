    ...
    while simulation_app.is_running():
        ...
        if count == 17:
            ...
            if count == 51:
                ...
                traj , _, _= generate_bimanual_arc_trajectories(picks[:, :, :3], garment.data.nodal_pos_w.clone().reshape(scene.num_envs, -1, 3), int(pnp_len), 0.25)
        ...
        if save == True:
            ...
            if count > 49:
                ...
                if count%10 == 0:
                    ...
                    # camera_data2 = scene["tiled_camera"].data.output["instance_id_segmentation_fast"]
                    # intrinsics = scene["tiled_camera"].data.intrinsic_matrices

                    points_3d_cam = unproject_depth(
                        scene["tiled_camera"].data.output["distance_to_image_plane"], scene["tiled_camera"].data.intrinsic_matrices
                    )
                    camera_data = transform_points(points_3d_cam, scene["tiled_camera"].data.pos_w, scene["tiled_camera"].data.quat_w_ros)
                    filename3 = save_folder + f"/camera/pixels/{time}" + ".pt"
                    # filename4 = save_folder + f"/camera/intrinsics/{time}" + ".pt"
                    # torch.save(camera_data2, filename3)
