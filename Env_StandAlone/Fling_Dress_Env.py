...
def FlingDress(pos, ori, usd_path, ground_material_usd, data_collection_flag, record_vedio_flag):
        
    env = FlingDress_Env(pos, ori, usd_path, ground_material_usd, record_vedio_flag)
    ...
    manipulation_points, indices, max_values = env.model.get_manipulation_points(input_pcd=pcd, index_list=[1900,1201])
    ...
    if data_collection_flag:
        for i in range(20):
            env.step()
        env.record(task_name="Fling_Dress", stage_index=1)  
    ...
    left_lift_points,right_lift_points=np.array([-0.1, 0.5, 1.1]), np.array([0.1, 0.5, 1.1]) 
    ...
    manipulation_points, indices, max_values = env.model.get_manipulation_points(input_pcd=pcd, index_list=[939,257,1614,334])
    ...
    if data_collection_flag:
        for i in range(20):
            env.step()
        env.record(task_name="Fling_Dress", stage_index=2)  
    ...
    manipulation_points[:,1]=1.3
    manipulation_points[:, 2] = 0.15
    env.bimanual_dex.dense_move_both_ik(left_pos=manipulation_points[0], left_ori=np.array([0.579, -0.579, -0.406, 0.406]), right_pos=manipulation_points[1], right_ori=np.array([0.406, -0.406, -0.579, 0.579]))
    ...
    if record_vedio_flag:
        if not os.path.exists("Data/Fling_Dress/vedio"):
            os.makedirs("Data/Fling_Dress/vedio")
        env.env_camera.create_mp4(get_unique_filename("Data/Fling_Dress/vedio/vedio", ".mp4"))
    ...
    if data_collection_flag:
        # write into .log file
        with open("Data/Fling_Dress/data_collection_log.txt", "a") as f:
    ...
    if data_collection_flag and success:
        env.record_to_npz()
        if not os.path.exists("Data/Fling_Dress/final_state_pic"):
            os.makedirs("Data/Fling_Dress/final_state_pic")
        env.env_camera.get_rgb_graph(save_or_not=True,save_path=get_unique_filename("Data/Fling_Dress/final_state_pic/img",".png"))
