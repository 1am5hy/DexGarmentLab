...
def HangDress(pos, ori, usd_path, env_dx, env_dy, ground_material_usd, data_collection_flag, record_vedio_flag):
    ...
    manipulation_points, indices, max_values = env.model.get_manipulation_points(input_pcd=env.garment_pcd, index_list=[1900,1201])
    ...
    lift_height=env.hanger_center[2]+garment_length/1.8
    ...
    if data_collection_flag:
        for i in range(20):
            env.step()
        env.record(task_name="Hang_Dress", stage_index=1)
    ...
    if data_collection_flag:
        # write into .log file
        with open("Data/Hang_Dress/data_collection_log.txt", "a") as f:
    ...
    if data_collection_flag:
        if success:
            ...
            if not os.path.exists("Data/Hang_Dress/final_state_pic"):
                os.makedirs("Data/Hang_Dress/final_state_pic")
            env.env_camera.get_rgb_graph(save_or_not=True,save_path=get_unique_filename("Data/Hang_Dress/final_state_pic/img",".png"))
