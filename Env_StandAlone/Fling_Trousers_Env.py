...
def FlingTrousers(pos, ori, usd_path, ground_material_usd, data_collection_flag, record_vedio_flag):
    
    env = FlingTrousers_Env(pos, ori, usd_path, ground_material_usd, record_vedio_flag)
    ...
    if data_collection_flag:
        for i in range(20):
            env.step()
        env.record(task_name="Fling_Trousers", stage_index=1)  
    ...
    # get lift points
    left_lift_points,right_lift_points=np.array([-distance-0.02, 0.4, 1.]), np.array([distance+0.02, 0.4, 1.]) 
    ...
    left_lift_points,right_lift_points=np.array([-distance-0.04, 1.4, 0.15]), np.array([distance+0.04, 1.4, 0.15])
    env.bimanual_dex.dense_move_both_ik(left_pos=left_lift_points, left_ori=np.array([0.579, -0.579, -0.406, 0.406]), right_pos=right_lift_points, right_ori=np.array([0.406, -0.406, -0.579, 0.579]))
    ...
    if data_collection_flag:
        # write into .log file
        with open("Data/Fling_Trousers/data_collection_log.txt", "a") as f:
    ...
    if data_collection_flag and success:
        env.record_to_npz()
        if not os.path.exists("Data/Fling_Trousers/final_state_pic"):
            os.makedirs("Data/Fling_Trousers/final_state_pic")
        env.env_camera.get_rgb_graph(save_or_not=True,save_path=get_unique_filename("Data/Fling_Trousers/final_state_pic/img",".png"))
